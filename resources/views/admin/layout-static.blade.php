...
@section("title", "layout-static - SB Admin")
...
<tr>
    <td>{{ $info->name }}</td>
     ...
     <td>
        @if($info->image)
            <img src="{{ asset($info->image) }}" alt="صورة الشيء المفقود" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px;">
        @else
      ...
      <td>
        <form action="{{ route('posts.toggle-delivery', $info->id) }}" method="POST">
            @csrf
            <button type="submit" class="btn {{ $info->is_delivered ? 'btn-danger' : 'btn-success' }} btn-sm text-white" style="border-radius: 20px; padding: 5px 15px;">
                {{ $info->is_delivered ? 'إلغاء التسليم' : 'تسليم' }}
            </button>
        </form>
      </td>
